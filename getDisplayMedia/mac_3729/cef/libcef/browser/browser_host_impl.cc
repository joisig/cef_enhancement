...
struct CefBrowserHostImpl::PendingAccessRequest {
    PendingAccessRequest(std::unique_ptr<Cef::DesktopMediaPicker> picker,
    ...
    std::unique_ptr<Cef::DesktopMediaPicker> picker;
...
void CefBrowserHostImpl::RequestMediaAccessPermission(
  ...
  if (request.video_type == blink::MEDIA_DISPLAY_VIDEO_CAPTURE){
        auto picker = picker_factory_->CreatePicker();
        if (!picker) {
            std::move(callback).Run(blink::MediaStreamDevices(), blink::MEDIA_DEVICE_INVALID_STATE, nullptr);
        }
        ...
        if (queue.size() == 1) { ProcessQueuedAccessRequest(queue, web_contents); }
  }
 else {
  //@zsf-e
...
void CefBrowserHostImpl::ProcessQueuedAccessRequest(
    ...
    Cef::DesktopMediaPicker::DoneCallback done_callback =
    base::BindRepeating(&CefBrowserHostImpl::OnPickerDialogResults,
                        base::Unretained(this), web_contents);
    
    Cef::DesktopMediaPicker::Params picker_params;
...
void CefBrowserHostImpl::OnPickerDialogResults(
    ...
    if (media_id.is_null()) {
        ...
        devices.push_back(blink::MediaStreamDevice(
                                                     blink::MEDIA_DISPLAY_VIDEO_CAPTURE, media_id.ToString(), media_id.ToString()));
    }
    
    std::move(pending_request.callback)
    .Run(devices, request_result, std::unique_ptr<content::MediaStreamUI>());
    queue.pop_front();
...
CefBrowserHostImpl::CefBrowserHostImpl(
    ...
    : content::WebContentsObserver(web_contents),
      ...
      //@zsf-b
      picker_factory_(new Cef::DesktopMediaPickerFactoryImpl())
      //@zsf-e
    {
